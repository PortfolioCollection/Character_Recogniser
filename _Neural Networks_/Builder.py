...
class Builder:
    def __init__(self, layer_sizes):
        ...
        self.net = Neaural_Net(layer_sizes)
    ...
    def generate_net(self):
        ...
        biases = np.random.uniform(-span,span,self.layer_sizes[0])
        self.net.add_node_layer(inputs, biases)
        #Hidden Layer and Output Layer
        for i in range(1,self.length):
            data = np.zeros(self.layer_sizes[i])
            biases = np.random.uniform(-span,span,self.layer_sizes[i])
            connections = np.random.uniform(-span,span,(self.layer_sizes[i],self.layer_sizes[i-1]))
